...
public:
    ...
    void move(double tau, unsigned int shag) {
        if((shag>10&& shag<20)||(shag>30&& shag<40)||(shag>50&& shag<60)||(shag>70&& shag<80)||(shag>90&& shag<100)) {
            x += -1.3*vx * tau;
            y += -1.3*vy * tau;
            z += -1.3*vz * tau;
        }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            //Здесь у меня две реализации трансформации геометрии,закомментированная просто превращает шатл в комету
            //Для просмотра закоментированного кода в Move нужно убрать условия после if
           /* double vz = (pow(pointX, 2) + pow(pointY, 2) + pow(pointZ, 2))*0.2;
            double vx =-pointX*0.2;
            double vy = -pointY*0.2;*/

            double vx =0;
            double vy = 0;
            double vz =0;
            if(abs(pointY)>1.1 && pointZ<0.3)
            {
                vz=1.5*pow((abs(pointY)-1.1),2);
            }
    ...
    void doTimeStep(double tau, unsigned int shag) {
        // По сути метод просто двигает все точки
        for(unsigned int i = 0; i < nodes.size(); i++) {
            nodes[i].move(tau,shag);
        }
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "C:\\Users\\darms\\CLionProjects\\Second_lab\\moving_objects\\vtk_data\\bandura" + std::to_string(snap_number) + ".vtu";
